...
- [x] **Portal path aliases**: Remove `@/auth/*` and `@/config/*` path aliases from `apps/portal/tsconfig.json` and replace all usages with `@atl/auth` and `@atl/config` imports directly (requires renaming imports across portal source)
