...
import { getUserRole, isAdmin, isAdminOrStaff } from "./check-role";
...
export type { SessionData } from "@atl/types/auth";
...
export const verifySession = cache(async (): Promise<SessionData> => {
  ...
  const { db } = await import("@atl/db/client");
  const { user } = await import("@atl/db/schema/auth");
