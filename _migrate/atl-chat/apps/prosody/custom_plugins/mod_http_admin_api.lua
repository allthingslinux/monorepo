-- mod_http_admin_api: Patched for Portal provisioning
-- Base: Prosody Community Modules (hg.prosody.im/prosody-modules)
-- Patch: Handle password in PUT /users/* for create/update (Portal integration)
-- Upstream update_user only handled display_name, role, roles, enabled; password was ignored.
...
function update_user(event, username)
	...
	-- PATCH: Handle password create/update (Portal provisioning)
	-- Upstream mod_http_admin_api ignored password; SCRAM auth failed after PUT.
	if new_user.password and type(new_user.password) == "string" and #new_user.password > 0 then
		local prepped = nodeprep(username);
		if not prepped or #prepped == 0 then
			return nil, "bad-request";
		end
		username = prepped;

		if usermanager.user_exists(username, module.host) then
			local ok, err = usermanager.set_password(username, new_user.password, module.host);
			if not ok then
				module:log("error", "Failed to set password for %s: %s", username, tostring(err));
				return nil, "internal-server-error";
			end
		else
			local ok, err = usermanager.create_user(username, new_user.password, module.host);
			if not ok then
				module:log("error", "Failed to create user %s: %s", username, tostring(err));
				return nil, "internal-server-error";
			end
		end
	end
