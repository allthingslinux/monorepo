...
const Values = memo(() => {
  return (
    <section className="py-8 sm:py-12 md:py-16 lg:py-20">
      <div className="mx-auto mb-6 max-w-3xl text-center sm:mb-8 md:mb-12">
        <h2 className="mb-3 font-bold text-xl tracking-tight sm:mb-4 sm:text-2xl md:text-3xl lg:text-4xl">
          Our Values
        </h2>
        <p className="px-0 text-muted-foreground text-sm sm:px-4 sm:text-base md:px-6">
          The principles that guide our organization and community.
        </p>
      </div>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 md:gap-5 lg:grid-cols-3 lg:gap-6">
        {valuesList.map((value) => {
          ...
          const cardContent = (
            <>
              <div
                className={`inline-flex items-center justify-center rounded-lg p-2 ${isHighlighted ? "bg-primary/20" : "bg-primary/10"} mb-3 sm:mb-4`}
              >
                <Icon
                  className={`size-5 sm:size-6 ${isHighlighted ? "text-primary" : "text-primary/80"}`}
                />
              </div>
              <h3 className="mb-1.5 flex items-center gap-1 font-semibold text-base sm:mb-2 sm:text-lg">
                {value.title}
                {isHighlighted && value.link && (
                  <ArrowUpRight className="inline-flex size-3.5 sm:size-4" />
                )}
              </h3>
          ...
          return isHighlighted && value.link ? (
            <Link
              className="block rounded-lg transition-all duration-200 hover:scale-[1.01] focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
              href={value.link}
              key={value.id}
            >
              <Card
                className={`relative h-full border p-3 sm:p-4 md:p-5 ${isHighlighted ? "border-primary/30 bg-card/50" : "border-border/50"} hover:shadow-sm`}
              >
                {cardContent}
              </Card>
            </Link>
            ...
            <Card
              className={`relative h-full border p-3 sm:p-4 md:p-5 ${isHighlighted ? "border-primary/30 bg-card/50" : "border-border/50"}`}
              key={value.id}
            >
              {cardContent}
            </Card>
