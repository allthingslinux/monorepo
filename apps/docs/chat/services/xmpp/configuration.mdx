...
| `PROSODY_OAUTH2_REGISTRATION_KEY`      | OAuth2 dynamic client registration key                   | _(must be set)_ |
...
| `PROSODY_DB_PASSWORD` | Database password | _(change before production)_ |
...
| Variable             | Description                    | Default                      |
| -------------------- | ------------------------------ | ---------------------------- |
| `TURN_SECRET`        | Shared secret with TURN server | _(change before production)_ |
